...
const App: React.FC = () => {
  const [messages, setMessages] = useState<ChatMessage[]>([
    {
      ...
      text: 'Local Core Online. All STARK HUD modules synchronized. Native Voice Array initialized. Awaiting command, Sir.',
      timestamp: Date.now(),
    }
  ]);
  ...
  // Initialize Native Web Speech API (No API Key Required)
  useEffect(() => {
    ...
    if (SpeechRecognition) {
      ...
      recognitionRef.current.onresult = (event: any) => {
        ...
        // Handle "Jarvis" wake word or active listening
        if (isListening) {
          handleSendMessage(transcript);
          stopListeningAction();
        } else if (transcript.includes('jarvis')) {
          const command = transcript.split('jarvis').pop()?.trim();
          if (command) {
      ...
      recognitionRef.current.onend = () => {
        // Keep the listener alive for the wake word
    ...
    <div className="h-screen w-screen p-6 flex flex-col gap-6 relative overflow-hidden text-cyan-400">
      ...
      <div className="flex justify-between items-center z-10 hud-border px-6 py-3 rounded-xl bg-slate-900/80">
        <div className="flex items-center gap-6">
          <div className="flex flex-col">
            ...
            <span className="text-[10px] tracking-[0.6em] opacity-40 uppercase">Native Voice Integration</span>
          </div>
          <div className="h-8 w-[1px] bg-cyan-500/20"></div>
          <div className="flex gap-4 text-[11px] font-bold">
            <div className="flex flex-col">
              <span className="opacity-30 text-[8px]">CONNECTION</span>
              <span className="text-green-400">LOCAL_ENCRYPTED</span>
            </div>
            <div className="flex flex-col">
              <span className="opacity-30 text-[8px]">STATUS</span>
              <span className="text-cyan-400">STANDALONE_MODE</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-8">
          ...
          <div className="w-12 h-12 rounded-full border border-cyan-500/30 flex items-center justify-center bg-cyan-500/5">
             <div className={`w-8 h-8 rounded-full border-t-2 border-cyan-400 ${isLoading ? 'animate-spin' : ''}`}></div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-grow flex gap-6 z-10 overflow-hidden">
        ...
        <div className="w-80 flex flex-col gap-6 h-full flex-shrink-0">
              ...
              {pendingUrl && (
                <div className="absolute inset-0 z-20 flex flex-col items-center justify-center backdrop-blur-md bg-slate-950/60 rounded-full border border-cyan-400/50 p-2 animate-[pulse_2s_infinite]">
                  ...
                  <button 
                    ...
                    className="w-24 h-24 rounded-full bg-cyan-500 text-black font-black uppercase text-[10px] flex items-center justify-center text-center leading-tight tracking-tighter hover:bg-white hover:text-cyan-600 transition-all shadow-[0_0_40px_rgba(34,211,238,0.9)] active:scale-90 border-4 border-white/20"
                  >
                    DEPLOY<br/>LINK
                  </button>
          ...
          <div className="hud-border rounded-xl p-6 flex-grow flex flex-col gap-6 overflow-y-auto custom-scrollbar bg-slate-900/60">
            ...
            {[
              { label: 'Core Throughput', value: stats.cpu, color: 'bg-cyan-500' },
              { label: 'Voice Processor', value: isListening || isSpeaking ? 92 : 0, color: 'bg-blue-500' },
              { label: 'Buffer Stability', value: stats.memory, color: 'bg-purple-500' },
              { label: 'Network Grid', value: stats.aiLoad, color: 'bg-emerald-500' }
            ].map(item => (
        ...
        <div className="w-96 flex flex-col gap-6 h-full flex-shrink-0">
          <div className="hud-border p-6 rounded-xl flex flex-col gap-4 bg-slate-900/60">
             <div className="text-[10px] uppercase opacity-60 tracking-[0.4em] border-b border-cyan-500/10 pb-2">Command Deck</div>
      ...
      <div className="z-10 flex justify-between items-center px-4 py-2 border-t border-cyan-500/10 text-[10px] font-bold opacity-40 uppercase tracking-[0.6em]">
        <div className="flex gap-10">
          ...
          <span>LOCATION: LOCAL_HOST_NODE</span>
        </div>
