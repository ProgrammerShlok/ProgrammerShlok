    ...
    <div className="h-screen w-screen p-4 md:p-8 flex flex-col gap-6 relative overflow-hidden text-cyan-400">
      ...
      <div className="flex justify-between items-start z-10">
        <div className="flex flex-col">
          <h1 className="text-2xl font-black tracking-tighter glow-text italic">STARK HUD v.6.0 [SECURE_GESTURE]</h1>
          <div className="flex gap-4 text-[10px] font-bold tracking-widest opacity-60">
            <span>CORE: ONLINE</span>
            <span className={isSpeaking ? 'text-green-400 animate-pulse' : ''}>VOICE: {isSpeaking ? 'TALKING' : 'IDLE'}</span>
            <span className={isListening ? 'text-cyan-400 animate-pulse' : ''}>MIC: {isListening ? 'LISTENING' : 'STANDBY'}</span>
          </div>
        </div>
      ...
      <div className="flex-grow flex flex-col lg:flex-row gap-6 z-10 overflow-hidden">
        ...
        <div className="w-full lg:w-1/4 flex flex-col gap-6 h-full overflow-hidden">
          ...
          <div className="hud-border rounded-xl p-4 flex-grow space-y-4 overflow-y-auto custom-scrollbar flex flex-col">
            <h2 className="text-xs uppercase tracking-[0.3em] opacity-70 mb-2 border-b border-cyan-500/20 pb-1">System Vitality</h2>
            {[
              { label: 'Neural Core', value: stats.cpu, color: 'bg-cyan-500' },
              { label: 'Voice Array', value: isListening || isSpeaking ? 85 : 12, color: 'bg-blue-500' },
              { label: 'Local Buffer', value: stats.aiLoad, color: 'bg-purple-500' }
            ].map(item => (
            ...
            {pendingUrl && (
              <div className="mt-auto pt-4 border-t border-cyan-500/20">
                <div className="p-3 border border-cyan-400 bg-cyan-400/5 rounded-lg animate-pulse">
                  <p className="text-[9px] uppercase font-bold text-center mb-2 tracking-widest text-cyan-300">External Node Ready</p>
                  <button 
                    onClick={handleOpenPendingUrl}
                    className="w-full py-4 bg-cyan-500 text-black font-black uppercase text-[11px] tracking-widest hover:bg-white transition-all shadow-[0_0_15px_rgba(34,211,238,0.5)] active:scale-95"
                  >
                    DEPLOY_BROWSER
                  </button>
                  <p className="text-[7px] opacity-40 text-center mt-2 uppercase tracking-tighter">Authentication required by system</p>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="w-full lg:w-1/3 flex flex-col gap-6 justify-end">
          <div className="hud-border p-4 rounded-xl flex flex-col gap-2 bg-cyan-950/10">
             <div className="text-[10px] uppercase opacity-60 tracking-widest mb-2 border-b border-cyan-500/10 pb-1">Quick Access Commands</div>
             <div className="grid grid-cols-2 gap-2">
               <button onClick={() => handleQuickAction('show photo of Tony Stark')} className="text-[10px] text-left border border-cyan-500/40 p-2 bg-cyan-500/5 hover:bg-cyan-500/20 transition rounded uppercase font-bold text-cyan-400">IMAGE_DATA</button>
               <button onClick={() => handleQuickAction('open measuring tool')} className="text-[10px] text-left border border-cyan-500/20 p-2 hover:bg-cyan-500/10 transition rounded uppercase font-bold">GRID_SCALE</button>
               <button onClick={() => handleQuickAction('open youtube')} className="text-[10px] text-left border border-cyan-500/20 p-2 hover:bg-cyan-500/10 transition rounded uppercase font-bold">MEDIA_NODE</button>
               <button onClick={() => handleQuickAction('search news')} className="text-[10px] text-left border border-cyan-500/20 p-2 hover:bg-cyan-500/10 transition rounded uppercase font-bold">DATA_NET</button>
             </div>
          </div>
      ...
      <div className="z-10 flex justify-between items-center text-[9px] font-bold opacity-30 uppercase tracking-[0.5em]">
        <span>Stark Industries // Secure User Gesture Protocol</span>
        <span>Independent Neural System v6.0</span>
      </div>
    </div>
  );
};
