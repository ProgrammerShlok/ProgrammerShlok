    ...
    <div className="flex flex-col h-full hud-border rounded-xl p-4 mono-font overflow-hidden relative">
      ...
      <div className="flex items-center justify-between border-b border-cyan-500/20 pb-2 mb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] text-cyan-400 opacity-50 ml-2 uppercase tracking-widest">STARK_TERMINAL_v5.7</span>
        </div>
      ...
      <div ref={scrollRef} className="flex-grow overflow-y-auto space-y-3 pr-2 custom-scrollbar">
        {messages.map((msg) => (
          <div key={msg.id} className={`flex flex-col ${msg.role === MessageRole.USER ? 'items-end' : 'items-start'}`}>
            ...
            <div className={`p-3 rounded-lg max-w-[85%] border shadow-sm relative group ${
      ...
      <div className="mt-4 relative group">
        ...
        <div className="relative">
          <span className="absolute left-2 top-1/2 -translate-y-1/2 text-cyan-500 text-sm font-bold">&gt;</span>
          <input
            type="text"
            value={inputValue}
            onChange={(e) => setInputValue(e.target.value)}
            onKeyDown={handleKeyDown}
            placeholder="TYPE COMMAND OR SAY 'JARVIS'..."
